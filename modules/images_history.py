...
def traverse_all_files(curr_path, image_list, search_txt, all_type=False):
    try:
    ...
    for file in f_list:
        ...
        if (not all_type) and (file[-4:] == ".txt" or file[-4:] == ".csv"):
            ...
            if search_pnginfo(file, search_txt):
                image_list.append(file)
        else:
            image_list = traverse_all_files(file, image_list, search_txt)
    return image_list

def auto_sorting(dir_name, search_txt):
    ...
    for file in f_list:
        ...
        if not is_valid_date(file):
            if file[-10:].rfind(".") > 0:
                ...
                files_list = traverse_all_files(file_path, files_list, search_txt, all_type=True)
...
def archive_images(dir_name, date_to, search_txt):
    filenames = []
    ...
    if False: #opts.images_history_reconstruct_directory:
        date_list = auto_sorting(dir_name, search_txt)
        for date in date_list:
            if date <= date_to:
                path = os.path.join(dir_name, date)
                if date == today and not os.path.exists(path):
                    continue
                filenames = traverse_all_files(path, filenames, search_txt)
            if len(filenames) > batch_size:
                break
        filenames = sorted(filenames, key=lambda file: -os.path.getmtime(file))
    else:
        filenames = traverse_all_files(dir_name, filenames, search_txt)
...
def show_images_history(gr: gr, opts, tabname, run_pnginfo, switch_dict):
    ...
    batch_date_to.change(archive_images, inputs=[img_path, batch_date_to, search_txt], outputs=change_date_output)
